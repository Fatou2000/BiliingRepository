...
class ForfaitResourceIT {
    ...
    void createForfait() throws Exception {
        ...
        // Create the Forfait
        ForfaitDTO forfaitDTO = forfaitMapper.toDto(forfait);
        restForfaitMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(forfaitDTO))
            )
    ...
    void putExistingForfait() throws Exception {
        ...
        updatedForfait
            ...
            .periode(UPDATED_PERIODE)
            .actif(UPDATED_ACTIF);
        ForfaitDTO forfaitDTO = forfaitMapper.toDto(updatedForfait);

        restForfaitMockMvc
            .perform(
                put(ENTITY_API_URL_ID, forfaitDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(forfaitDTO))
            )
    ...
    void patchNonExistingForfait() throws Exception {
        ...
        restForfaitMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, forfaitDTO.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(forfaitDTO))
            )
    ...
    void deleteForfait() throws Exception {
        ...
        // Delete the forfait
        restForfaitMockMvc
            .perform(delete(ENTITY_API_URL_ID, forfait.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
